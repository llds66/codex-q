...
const visibleUsageItems = computed(() =>
  view.value === "project"
    ? usageItems.value.slice(
        projectPage.value * projectPageSize,
        (projectPage.value + 1) * projectPageSize,
      )
    : usageItems.value,
)
...
const weekResetLabel = computed(() => `7d重置时间：${formatResetTime(usage.value?.week?.resetsAt)}`)
...
onKeyDown((key) => {
  if (key.name === "q") exit()
    ...
    projectPage.value =
      (projectPage.value + direction + projectPageCount.value) % projectPageCount.value
  } else if (key.name === "r") {
    void refreshUsage()
  }
})
